  ...
  return <main className="arcade">
    ...
    <div className="game-shell">
      ...
      <div className="workspace">
        ...
        <div className="previews"><div className="view-toolbar"><span><i aria-hidden="true">⌘</i> 两图联动 <b>{Math.round(view.zoom * 100)}%</b></span><div className="actions"><button aria-label="缩小视图" onClick={() => setView(previous => zoomView(previous, previous.zoom / 1.25))} disabled={view.zoom <= 1}>−</button><button aria-label="放大视图" onClick={() => setView(previous => zoomView(previous, previous.zoom * 1.25))} disabled={view.zoom >= 8}>＋</button><button onClick={() => setView(initialView)}>重置视图</button></div></div>
          <section className="board-panel game-panel target-panel"><header className="panel-heading"><div><span className="micro">TARGET / 0{levelNumber}</span><h2>目标图 · {level.title}</h2></div><span className="tag">{size} × {size}</span></header><div className="board-body"><PixelCanvas key={`target-${levelId}`} colors={target} radius={level.radius} label="目标图画布" view={view} setView={setView}/><div className="board-info"><div className="target-emblem"><LevelGlyph kind={level.id}/></div><span className="micro">MISSION</span><h3>复现目标图</h3><p>让每一格<br/>都与目标一致。</p><dl><dt>坐标范围</dt><dd>−{level.radius} ～ {level.radius}</dd><dt>原点</dt><dd>画布中心</dd></dl></div></div></section>
          <section className={`board-panel game-panel result-panel ${work?.passed && !isHistorical ? 'is-cleared' : ''}`}><header className="panel-heading"><div><span className="micro">YOUR CREATION</span><h2>我的作品</h2></div><span className="tag">{status === 'running' ? '绘制中' : work ? isHistorical ? '历史结果' : '已生成' : '待运行'}</span></header><div className="board-body"><PixelCanvas key={`work-${levelId}`} colors={work?.colors ?? blank} radius={level.radius} label="学生作品画布" view={view} setView={setView}/><div className="board-info result-info">{work ? <><div className={work.passed ? 'clear-emblem' : 'match-emblem'} aria-hidden="true">{work.passed ? '★' : '◇'}</div><div className={`score ${work.passed ? 'success' : ''}`} data-testid="score"><span className="micro">匹配率</span><strong>{work.percent.toFixed(1)}<em>%</em></strong><span className="score-label">{work.passed ? '通关！' : '尚未匹配'}</span><small>{Math.round(work.elapsedMs)} ms</small></div><div className="match-meter" aria-hidden="true"><i style={{ width: `${work.percent}%` }}/></div>{isHistorical && <p className="historical">当前显示上次成功运行的结果，请以重新运行为准。</p>}</> : <><div className="waiting-pixels" aria-hidden="true"><i/><i/><i/><i/></div><span className="micro">READY TO CREATE</span><h3>轮到你了</h3><p>写下你的代码，<br/>点击运行生成作品。</p><span className="pending-score">匹配率 —</span></>}</div></div></section>
        </div>
      </div>
    </div>
    <footer className="game-footer"><span><b>●</b> 本地存档 · 自动保存</span><span>滚轮缩放 / 拖动平移 / 悬停查坐标</span><span className="micro">SYSTEM ONLINE / PYTHON ENGINE</span></footer>
