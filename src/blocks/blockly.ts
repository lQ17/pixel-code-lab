...
Blockly.common.defineBlocksWithJsonArray([
  ...
  { type: 'voxel_entry', message0: '决定体素颜色（x，y，z）', message1: '%1', args1: [statement('BODY')], colour: 195, tooltip: '系统遍历每个三维坐标，执行里面的规则；返回 0 时不放置体素。' },
  { type: 'voxel_coord', message0: '坐标 %1', args0: [dropdown('AXIS', [['x', 'x'], ['y', 'y'], ['z', 'z']])], output: 'Number', colour: 195 },
...
for (const type of Object.keys(Blockly.Blocks).filter(type => type.startsWith('pixel_') || type.startsWith('voxel_'))) {
  const original = Blockly.Blocks[type].init
  Blockly.Blocks[type].init = function(this: Blockly.Block) {
    original.call(this)
    this.contextMenu = false
    if (type === 'pixel_entry' || type === 'voxel_entry') { this.setDeletable(false); this.isDuplicatable = () => false }
  }
}
const item = (type: string) => ({ kind: 'block', type })
export const toolboxFor = (mode: SpaceMode) => ({ kind: 'categoryToolbox', contents: [
  { kind: 'category', name: '颜色', colour: '30', contents: [item('pixel_return'), item('pixel_color')] },
  { kind: 'category', name: '坐标', colour: '195', contents: [item(mode === '3d' ? 'voxel_coord' : 'pixel_coord')] },
