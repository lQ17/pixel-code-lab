...
export function HelpDialog({ onClose }: { onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="help-dialog" aria-labelledby="help-title" onCancel={event => { event.preventDefault(); onClose() }}>
    ...
    <ol><li>系统会遍历画布的每个整数坐标，调用一次 <code>pixel(x, y)</code>。你无需自己编写遍历循环。</li><li>函数返回 0～8 的整数决定该格颜色；0 表示空白，在画布上显示为深色底格。调色模块始终可见。</li><li>原点在中心，x 向右、y 向上。悬停可以查看坐标和颜色；滚轮缩放，拖动画布平移，两图同步。</li><li>点击运行即可看到作品和匹配率。图案完全一致就通关，三关随时可选。</li></ol>
