...
## 功能
- 二维自由创作支持 Python / 积木两种编辑方式。中文积木包含坐标、整数、颜色、运算、判断和返回；可查看生成 Python，使用同一执行器运行。两份草稿独立保存，切换不覆盖。
- 积木作品保存可编辑结构，支持作品库、JSON、缩略图和 PNG；「复制为 Python 作品」生成独立文本副本。旧 Python 文件兼容，积木文件使用 version=2。首版仅二维自由创作，不支持 Python 反向转积木。
