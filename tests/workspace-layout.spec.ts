...
// Regression coverage for the workspace refactor follow-up.
test('三维创作参考选中标记独立于挑战，帮助说明跟随当前玩法', async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem('pixel-code-lab.progress', JSON.stringify({
    schemaVersion: 1, codes: {}, passed: {}, levelId: 'square', introSeen: true,
    voxelLevelId: 'voxel-cube', voxelReferenceId: 'voxel-house', voxelCodes: {},
  })))
  ...
  await expect(page.getByRole('menuitemradio', { name: /简单房屋/ })).toHaveAttribute('aria-checked', 'true')
  await expect(page.getByRole('menuitemradio', { name: /^\s*立方体$/ })).toHaveAttribute('aria-checked', 'false')
  await page.getByRole('menuitemradio', { name: /球体/ }).click()
  await expect(page.locator('.target-panel h2')).toContainText('球体')
  await page.getByRole('button', { name: /参考 ▾/ }).click()
  await expect(page.getByRole('menuitemradio', { name: /球体/ })).toHaveAttribute('aria-checked', 'true')
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: /帮助 ▾/ }).click()
  await page.getByRole('menuitem', { name: /使用说明/ }).click()
  const help = page.getByRole('dialog')
  await expect(help).toContainText('voxel(x, y, z)')
  await expect(help).toContainText('自由创作不计算匹配率')
  await expect(help).toContainText('编辑 → 颜色表')
  await expect(help).not.toContainText('move_origin')
  await expect(help).not.toContainText('调色模块始终可见')
  await page.screenshot({ path: 'test-results/help-3d-create.png' })
  ...
  await page.getByRole('button', { name: /帮助 ▾/ }).click()
  await page.getByRole('menuitem', { name: /使用说明/ }).click()
  await expect(help).toContainText('pixel(x, y)')
  await expect(help).toContainText('move_origin')
  await expect(help).not.toContainText('载入示例')
})
