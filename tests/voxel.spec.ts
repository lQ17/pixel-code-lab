...
test('三维示例、视角、错误恢复、独立存档和模式取消', async ({ page, context }) => {
  ...
  await page.getByRole('button', { name: '3D 体素创作', exact: true }).click()
  ...
  await page.getByRole('button', { name: /立方体.*载入示例/ }).click()
  await run.click()
  ...
  await page.getByRole('button', { name: /球体.*载入示例/ }).click()
  await run.click()
