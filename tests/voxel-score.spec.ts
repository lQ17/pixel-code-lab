...
test('三维成绩、剖切无关、独立通关、失败保留、刷新恢复',async({page,context})=>{
 ...
 await page.setViewportSize({width:1440,height:900});await page.goto('/');await page.getByRole('button',{name:'开始挑战',exact:true}).click();await page.getByRole('button',{name:'3D 体素',exact:true}).click(); await page.getByRole('button',{name:'挑战',exact:true}).click()
 ...
 await page.getByRole('button', { name: /^立方体/ }).click(); await page.getByRole('button', { name: '载入示例', exact: true }).click();await run.click();await expect(score).toContainText('100.0%');await expect(page.getByRole('button',{name:/^立方体.*已通关/})).toBeVisible()
