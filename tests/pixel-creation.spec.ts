...
async function seed(page: Page, value: any = base) {
  await page.addInitScript(({ key, value }) => {
    if (!localStorage.getItem(key)) localStorage.setItem(key, JSON.stringify(value))
  }, { key: STORAGE_KEY, value })
  const targetMode = value?.mode === '3d' ? '3d' : '2d'
  await page.goto(`/#/work/${targetMode}/create`)
...
test('二维创作、原点、缩略图、透明 PNG、刷新与挑战三维隔离', async ({ page, context }) => {
  ...
  const data = await png(page, () => clickExportPng(page))
...
test('二维参考只改上图，示例与 Python 结果一致，草稿备份和导入导出', async ({ page }) => {
  ...
  for (const id of pixelReferenceIds) {
    ...
    await switchReference(page, titles[id])
    page.once('dialog', d => d.accept())
...
test('三维完整缩略图与 PNG 不受剖切、旋转和参考影响，旧作品可补预览', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '关闭作品库' }).click()
  await run(page)
  const full = await png(page, () => clickExportPng(page))
  await page.getByLabel('三维体素画布').last().press('ArrowLeft')
  await toggleVoxelCutHandles(page)
  const handle = page.getByRole('slider').first()
  await handle.press('Home')
  const cut = await png(page, () => clickExportPng(page))
